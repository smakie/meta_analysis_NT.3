...
######################################################################
#old function - replaced by rma.mv.outlier
rma.mv.outlier_old <- function(data, struct = "CS", method = "REML", random, mods = NULL){
  data1 <- rowid_to_column(data, var = "effect_no1")
  leave1out <- c()
  se <- c()
  tval <- c()
  pval <-c()
  ci.lb <- c()
  ci.ub <- c()
  for(counter in 1:max(data1$effect_no1)){
    modresult <- rma.mv(yi = yi, V = vi, data = filter(data1, effect_no1 != counter), struct = struct, method = method, test = "t", 
                                      random = random, mods = mods)
    leave1out <- c(leave1out , modresult$b)
    se <- c(se, modresult$se)
    tval <- c(tval, modresult$tval)
    pval <- c(pval, modresult$pval)
    ci.lb <- c(ci.lb, modresult$ci.lb)
    ci.ub <- c(ci.ub, modresult$ci.ub)
  }
  return(mutate(add_column(as.data.frame(cbind(leave1out, se, tval, pval, ci.lb, ci.ub)), select(meta_tib, effect), .before = T), 
                sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")))
}
######################################################################
#old function - replaced by rma.mv.outlier
rma.mv.outlier_old2 <- function(data, struct = "CS", method = "REML", random, mods = NULL, b = 0){
  data1 <- rowid_to_column(data, var = "effect_no")
  data3 <- mutate(data1, effect_no = as.factor(effect_no))
  model0 <- rma.mv(yi = yi, V = vi, data = data1, struct = struct, method = method, test = "t", 
                                      random = random, mods = mods)
  leave1out <- c()
  se <- c()
  tval <- c()
  ...
  ci.lb <- c()
  ci.ub <- c()
  for(counter in 1:max(data1$effect_no)){
    data2 <- filter(data1, effect_no != counter)
    model <- rma.mv(yi = yi, V = vi, data = data2, struct = struct, method = method, test = "t", 
                                      random = random, mods = mods)
  ...
  cd1 <- cooks.distance(model0, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = data3$effect_no)
  cd2 <- rename(mutate(rownames_to_column(as.data.frame(cd), var = "effect_no"), effect_no = as.factor(effect_no)), cooks.d = cd)
  return(full_join(mutate(add_column(add_column(as.data.frame(cbind(leave1out, se, tval, pval, ci.lb, ci.ub)), select(meta_tib, effect), .before = T), mutate(select(data1, effect_no), effect_no = as.factor(effect_no)), .before = T), 
                sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")), cd2, by = "effect_no"))
}


######################################################################
#This is replaced by rma.mv.outlier
rma.mv.outlier1 <- function(data, struct = "CS", method = "REML", random, mods = NULL, b = 0, cluster){
  c <- cluster
  data1 <- rowid_to_column(data, var = "effect_no")
  model0 <- rma.mv(yi = yi, V = vi, data = data1, struct = struct, method = method, test = "t", 
                                      random = random, mods = mods)
  ...
  cd.2 <- rename(mutate(rownames_to_column(as.data.frame(cd.1), var = "effect"), effect = as.factor(effect)), cooks.d = cd.1)
  dfbetas1 <- dfbetas(model0, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = as.factor(c))
  dfbetas2 <- mutate(rownames_to_column(as.data.frame(t(as.data.frame(t(dfbetas1))[(b + 1),])), var = "effect"), effect = as.factor(effect))
  names(dfbetas2)[2]<-paste("DFBETAS") 
  return(select(full_join(full_join(mutate(add_column(as.data.frame(cbind(leave1out, se, tval, pval, ci.lb, ci.ub, model.aic, model.bic, model.aicc)), rename(as.tibble(as.factor(c)), effect = value), .before = T),
                sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")), cd.2, by = "effect"), dfbetas2, by = "effect"), effect, cooks.d, DFBETAS, leave1out, se, tval, pval, ci.lb, ci.ub, sig, model.aic, model.bic, model.aicc))
}


######################################################################
#This is replaced by rma.mv.outlier (they are they same actually)
rma.mv.outlier.study <- function(data, struct = "CS", method = "REML", random, mods = NULL, b = 0, cluster){
  data.gojira <- data
  c <- cluster
  data.gojira.2 <- rowid_to_column(mutate(data.gojira, study_variable.gojira = as.factor(c)), var = "rowid")
  data.gojira.2$study_identification.gojira <- group_indices(group_by(data.gojira.2, study_variable.gojira), study_variable.gojira)
  model0 <- rma.mv(yi = yi, V = vi, data = data.gojira.2, struct = struct, method = method, test = "t", 
                                      random = random, mods = mods) 
  study <- c()
  leave1out <- c()
  se <- c()
  ...
  model.bic <- c()
  model.aicc <- c()
  for(counter in 1:max(data.gojira.2$study_identification.gojira)){
    data.gojira.3 <- filter(data.gojira.2, study_identification.gojira != counter)
    model <- rma.mv(yi = yi, V = vi, data = data.gojira.3, struct = struct, method = method, test = "t", 
                                      random = random, mods = mods)
    study <- c(study, as.vector(select(filter(data.gojira.2, study_identification.gojira == counter) , study_variable.gojira)[1,]))
    leave1out <- c(leave1out, model$b[b+1])
    se <- c(se, model$se[b+1])
    tval <- c(tval, model$zval[b+1])
    pval <- c(pval, model$pval[b+1])
  ...
  cd.1 <- cooks.distance(model0, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = c)
  cd.2 <- rename(mutate(rownames_to_column(as.data.frame(cd.1), var = "study"), study = as.factor(study)), cooks.d = cd.1)
  dfbetas1 <- dfbetas(model0, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = c)
  dfbetas2 <- mutate(rownames_to_column(as.data.frame(t(as.data.frame(t(dfbetas1))[(b + 1),])), var = "study"), study = as.factor(study))
  names(dfbetas2)[2]<-paste("DFBETAS") 
  return(select(mutate(full_join(full_join(mutate(mutate(as.data.frame(cbind(study, leave1out, se, tval, pval, ci.lb, ci.ub, model.aic, model.bic, model.aicc)),
                sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")), leave1out = as.numeric(leave1out), se = as.numeric(se), tval = as.numeric(tval), pval = as.numeric(pval), ci.lb = as.numeric(ci.lb),ci.ub = as.numeric(ci.ub),model.aic = as.numeric(model.aic), model.bic = as.numeric(model.bic),  model.aicc = as.numeric(model.aicc)), cd.2, by = "study"), dfbetas2, by = "study"), study = as.factor(study)), study, cooks.d, DFBETAS, leave1out, se, tval, pval, ci.lb, ci.ub, model.aic, model.bic, model.aicc))
}

######################################################################
rma.mv.outlier.old3 <- function(data, struct = "CS", method = "REML", random = NULL, mods = NULL, b = 0, cluster){
  data.gojira <- data
  c <- cluster
  data.gojira.2 <- mutate(data.gojira, cluster.gojira = as.factor(c))
  ...
  names(dfbetas2)[2]<-paste("DFBETAS") 
  return(select(mutate(full_join(cluster_unit_col2, full_join(full_join(mutate(mutate(as.data.frame(cbind(cluster_unit, leave1out, se, tval, pval, ci.lb, ci.ub, model.aic, model.bic, model.aicc)),
                sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")), cluster_unit = as.factor(cluster_unit),leave1out = as.numeric(leave1out), se = as.numeric(se), tval = as.numeric(tval), pval = as.numeric(pval), ci.lb = as.numeric(ci.lb),ci.ub = as.numeric(ci.ub),model.aic = as.numeric(model.aic), model.bic = as.numeric(model.bic),  model.aicc = as.numeric(model.aicc)), cd.2, by = "cluster_unit"), dfbetas2, by = "cluster_unit"), by = "cluster_unit"), cluster_unit = as.factor(cluster_unit)), cluster_unit, cooks.d, DFBETAS, leave1out, se, tval, pval, ci.lb, ci.ub, model.aic, model.bic, model.aicc))
}

######################################################################
#
rma.mv.outlier <- function(data, struct = "CS", method = "REML", random = NULL, mods = NULL, b = 0, cluster){
  data.gojira <- data
  c <- cluster
  data.gojira.2 <- mutate(data.gojira, cluster.gojira = as.factor(c))
  data.gojira.2$cluster.gojira2 <- group_indices(group_by(data.gojira.2, cluster.gojira), cluster.gojira)
...
rma.mv.outlier.new <- function(data, struct = "CS", method = "REML", random = NULL, mods = NULL, b = 0, cluster, robust = FALSE, robust.cluster, adjust = FALSE){
  data.gojira <- data
  c <- cluster
  data.gojira.2 <- mutate(data.gojira, cluster.gojira = as.factor(c))
  data.gojira.2$cluster.gojira2 <- group_indices(group_by(data.gojira.2, cluster.gojira), cluster.gojira)
  ...
  for(counter in 1:max(data.gojira.2$cluster.gojira2)){
    ...
    if (isFALSE(robust)){
      ...
      model <- metafor::robust(model.p1, cluster = robust.cluster, adjust = adjust)}
    cluster_unit <- c(cluster_unit, as.vector(select(filter(data.gojira.2, cluster.gojira2 == counter) , cluster.gojira)[1,]))
    leave1out <- c(leave1out, model$b[b+1])
    se <- c(se, model$se[b+1])
    tval <- c(tval, model$zval[b+1])
...
something2 <- rma.mv(yi = yi, V = vi, mods = ~NT_type.1, random = ~1 | study_id/effect, 
    struct = "CS", data = meta_tib, method = "REML", test = "t")
rma.mv.outlier.new(data = meta_tib, struct = "CS", method = "REML", random = ~1 | study_id/effect, mods = ~NT_type.1, b = 0, cluster = meta_tib$effect, robust = T, robust.cluster = meta_tib$study_id, adjust = TRUE)
something2$ci.lb


data.gojira <- meta_tib
  c <- meta_tib$effect
  data.gojira.2 <- mutate(data.gojira, cluster.gojira = as.factor(c))
  data.gojira.2$cluster.gojira2 <- group_indices(group_by(data.gojira.2, cluster.gojira), cluster.gojira)
  data.gojira.3 <- filter(data.gojira.2, cluster.gojira2 != 1)
meta_tib$effect

robust.cluster <- "effect"
data.gojira.3$effect
data.gojira.3 %>% select(robust.cluster) %>% as.data.frame() %>% as.list()

something <- metafor::robust(ml_meta_model.3l_mod_NT_type.1, cluster=meta_tib$effect, adjust = T)
something$fit.stats[2][3,]

cooks.distance(something, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$effect)
```
...
#The effect level (Level 2)
#Cook's d
ml_meta_model.3l_cooks.d_lev2 <- cooks.distance(ml_meta_model.2lre, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$effect) %>% as.data.frame() %>% rownames_to_column(var = "effect") %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
...
#DFBETAS
#Effect level 
ml_meta_model.3l_dfbetas_lev2 <- dfbetas(ml_meta_model.3l, cluster = meta_tib$effect, reestimate = T) %>% rownames_to_column(var = "effect") %>% arrange(desc(abs(intrcpt)))
ml_meta_model.3l_dfbetas_lev2
#RESULTS 
#No influential cases

#The Study level (Level 3)
#Cook's d 
ml_meta_model.3l_cooks.d_lev3 <- cooks.distance(ml_meta_model.2lre, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$study_id) %>% as.data.frame() %>% rownames_to_column(var = "effect") %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
...
#DFBETAS
ml_meta_model.3l_dfbetas_lev3 <- dfbetas(ml_meta_model.3l, cluster = meta_tib$study_id, reestimate = T) %>% rownames_to_column(var = "effect") %>% arrange(desc(abs(intrcpt)))
ml_meta_model.3l_dfbetas_lev3 
#RESULTS 
#-- No influential cases

#Leave-1-out 
#Effect level 
ml_meta_model.3l_outlier.lev2 <- rma.mv.outlier(meta_tib, struct = "CS",method = "REML", random = ~1 | study_id/effect, cluster = meta_tib$effect)
ml_meta_model.3l_outlier.lev2
ml_meta_model.3l_outlier.lev2 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
#There are no outliers and influential cases

#Study level
ml_meta_model.3l_outlier.lev3 <- rma.mv.outlier(meta_tib, struct = "CS",method = "REML", random = ~1 | study_id/effect, cluster = meta_tib$study_id)
ml_meta_model.3l_outlier.lev3
ml_meta_model.3l_outlier.lev3 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
#There are no outliers and influential cases
...
#2-level re
ml_meta_model.2lre_mod_main_language <- rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "REML", test = "t",
                                          mods = ~ main_language,
                                          random = ~ 1 | effect)
ml_meta_model.2lre_mod_main_language

#2-level model 
ml_meta_model.2l_mod_main_language <- rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "REML", test = "t",
                                          mods = ~ main_language,
                                          random = ~ 1 | study_id)
...
#Study level 
dfbetas(ml_meta_model.3l_mod_main_language, progbar=FALSE,reestimate=TRUE, parallel="no", ncpus=1, cluster= meta_tib$study_id) %>% 
  mutate(absDFBETAS = abs(main_language1)) %>% 
  arrange(desc(absDFBETAS))
#DFBETAS RESULTS
#-- All DFBETAS are well below 1
...
#Beta 1
#-- Effect level 
ml_meta_model.3l_mod_main_language_outlier.lev2.b1 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ main_language, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 1)
ml_meta_model.3l_mod_main_language_outlier.lev2.b1
ml_meta_model.3l_mod_main_language_outlier.lev2.b1 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
#There are no outliers and influential cases

#-- Study level
ml_meta_model.3l_mod_main_language_outlier.lev3.b1 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ main_language, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 1)
ml_meta_model.3l_mod_main_language_outlier.lev3.b1
ml_meta_model.3l_mod_main_language_outlier.lev3.b1 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
#There is an influential study (boonen_et_al.2011)

refit_mod_main_language <- rma.mv(yi = yi, V = vi, data = filter(meta_tib, study_id != "boonen_et_al.2011"), struct = "CS", method = "REML", mods = ~ main_language, random = ~1 | study_id/effect, test = "t")
regplot(ml_meta_model.3l_mod_main_language, xlab = "Language")
regplot(refit, xlab = "Language")
#Removing Boonen's study resulted in a stronger effect (and becomes significant), however, data contains only 2 studies using Dutch, for now, I conclude that there is no language effect, but but need to do more study on other languages 
...
#rma.mv(yi = yi, V = vi, data = filter(meta_tib, effect != c("boonen_et_al.2011b", "boonen_et_al.2011d")), struct = "CS", method = "REML", test = "t",mods = ~ main_language,random = ~ 1 | study_id)
...
#Effect level
#Beta 1
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b1 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b1 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b1 
#Based on Cook's d and DFBETAS, There are no outliers and influential cases 

#Beta 2
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b2 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 2)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b2 
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b2  %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
#Based on Cook's d and DFBETAS, There are no outliers and influential cases 

#Beta 3
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b3 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 3)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b3 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b3
#Based on Cook's d and DFBETAS, There are no outliers and influential cases 

#Beta 4
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b4 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 4)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b4 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b4
#Based on Cook's d and DFBETAS, There are no outliers and influential cases 

#Beta 5
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b5 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 5)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b5 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b5
#Based on Cook's d and DFBETAS, There are no outliers and influential cases 

#Beta 6
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b6 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 6)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b6 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b6
#Based on Cook's d and DFBETAS, There are no outliers and influential cases 

#Beta 7
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b7 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 7)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b7 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
...
#STUDY LEVEL
#Beta 1
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b1 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b1 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b1 %>% filter(abs(DFBETAS) >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b1 
#Based on DFBETAS, there are no influential studies 

#Beta 2
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b2 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 2)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b2 %>% filter(abs(DFBETAS) >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b2  %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
##Based on DFBETAS, there is 1 influential study (casey_et_al.2018)

#Beta 3
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b3 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 3)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b3 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b3 %>%filter(abs(DFBETAS) >= 1)
#Based on DFBETAS, there are no influential studies 

#Beta 4
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b4 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 4)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b4 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b4 %>% filter(abs(DFBETAS) >= 1)
#Based on DFBETAS, there are no influential studies 

#Beta 5
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b5 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 5)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b5 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b5 %>% filter(abs(DFBETAS) >= 1)
#Based on DFBETAS, there are no influential studies 

#Beta 6
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b6 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 6)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b6 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b6 %>% filter(abs(DFBETAS) >= 1)
#Based on DFBETAS, there are no influential studies 

#Beta 7
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b7 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 7)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b7 %>% filter(cooks.d >= 1 | abs(DFBETAS) >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b7 %>% filter(abs(DFBETAS) >= 1)
#There are no outliers and influential cases 



#NT_type.1cardinalityNT_vs_mean
...
refit_mod_NT_type.1 <- rma.mv(yi = yi, V = vi, data = filter(meta_tib, study_id != "casey_et_al.2018"), struct = "CS", method = "REML", test = "t",
                                      mods = ~ NT_type.1, 
                                      random = ~ 1 | study_id/effect)
refit_mod_NT_type.1
#SIGNIFICANT RESULTS
#INTERCEPT = 	-0.0241
#NT that have significantly greater effect 
#--- Cardinality (b = 0.1824, p = 0.1990)
#--- Conventional_Nominatives (b = 0.4900, p = <.0001)
#NT that have significantly negative effect 
#--- Calculation (b = -0.2487, p = 0.0536)


#Removing casey_et_al.2018 resulted in decreasing the effect of calculation NT and cardinality NT (and becomes non-significant) but increased the effect of conventional nominatives
#Conclusion 
#It is safe to suggest that conventional nominatives NT is the best type of NT
#Cardinality talk does not have a significant effect, however, because it also does not have a significant negative effect, and studies show that NT variety also has a positive effect, we should also include cardinality talk 
#Calculation NT: Need further research, but I recommend not to include calculation NT

#Standardised residuals for outliers 
ml_meta_model.3l_resid %>% mutate(std.resid = resid/m)
ml_meta_model.3l


residuals(ml_meta_model.3l) %>% as.data.frame() %>% rownames_to_column(., var = "effect") %>% rename(., resid = .) %>% mutate(std.resid = resid/sd) %>% filter(std.resid >= qnorm(p = 0.975))
...
#Histogram
#Finding the sd to calculate the standardised residuals 
sd <- vardist.3l(model = ml_meta_model.3l, digits =  10)$variances %>% sum() %>% sqrt()
#Residuals and standardised residuals
ml_meta_model.3l_mod_NT_type.1_resid <- residuals(ml_meta_model.3l_mod_NT_type.1) %>% as.data.frame() %>% rownames_to_column(., var = "effect") %>% rename(., residuals = .) %>% mutate(std.resid = residuals/sd)
ml_meta_model.3l_mod_NT_type.1_resid 
ggplot2::ggplot(data = ml_meta_model.3l_resid) + geom_histogram(mapping = aes(x= resid),binwidth = 0.06, alpha = 0.8, fill= "#589E9E", color = "#000000") + theme_minimal() 
qqnorm(residuals(ml_meta_model.3l))
#RESULTS 
#Normality of residuals is met 


#NORMALITY OF THE SAMPLING DISTRIBUTION 
#Sensitivity test with bootstrap sampling 




#Oultier based on residuals 
#Normality of residuals 
#Normality of sampling distribution (Sensitivity analysis)



#HETEROSCEDASTICITY 
#Making the Residuals/fitted plot
#Residuals
ml_meta_model.3l_mod_NT_type.1_resid #(see above)
#Fitted values
ml_meta_model.3l_mod_NT_type.1.fitted <- fitted(ml_meta_model.3l_mod_NT_type.1) %>% as.data.frame() %>% rownames_to_column(., var = "effect") %>% rename(., fitted.values = .)
#Residuals and fitted values data
ml_meta_model.3l_mod_NT_type.1.resid.fitted <- full_join(ml_meta_model.3l_mod_NT_type.1_resid, ml_meta_model.3l_mod_NT_type.1.fitted, by = "effect")

#Making the horizontal line
xzerox <- seq(-0.1, 0.4, by = 0.5) %>% as.data.frame() %>% rename(., xzerox = .)
xzerox 
yzerox <- rep(c(0),times= nrow(xzerox)) %>% as.data.frame() %>% rename(., yzerox = .)
yzerox
xyzerox <- cbind(xzerox, yzerox)

#Heteroscedasticity 
ggplot(data = NULL) + geom_point(data = ml_meta_model.3l_mod_NT_type.1.resid.fitted, mapping = aes(x = fitted.values, y = residuals), color = "#589E9E", alpha = 0.6, size = 3) + geom_line(data = xyzerox, col = "black", size = 0.5,mapping = aes(x = xzerox, y = yzerox)) + scale_x_continuous(limits = c(-0.1, 0.4)) + theme_minimal()
#RESULTS 
...
#Bonf for cardinality 
0.0819

ml_meta_model.3l_mod_NT_type.1.rbst$beta %>% as.data.frame() %>% rownames_to_column(., var = "comparison") %>% rename(., estimate = V1) %>% add_column(rename(as.tibble(ml_meta_model.3l_mod_NT_type.1.rbst$pval), pval = value)) %>% filter(comparison != "intrcpt") %>% mutate(bonf.adjusted.pval = pval*nrow(.))


ml_meta_model.3l_mod_NT_type.1.rbst$beta %>% as.data.frame() %>% rownames_to_column(., var = "comparison") %>% rename(., estimate = V1) %>% add_column(rename(as.tibble(ml_meta_model.3l_mod_NT_type.1.rbst$pval), pval = value)) %>% filter(comparison != "intrcpt") %>% arrange(desc(pval)) %>% rowid_to_column(var = "rank") %>% select(comparison, pval, rank) %>% arrange(pval) %>% mutate(adjusted.crit.p = 0.05/rank) %>% mutate(adjusted.sig = ifelse(pval <= adjusted.crit.p, "sig", "ns"))
...
#Assumption checks 
#Normality of residuals 
ml_meta_model.3l_resid <- residuals(ml_meta_model.3l) %>% as.data.frame() %>% rownames_to_column(., var = "effect") %>% rename(., resid = .)
ml_meta_model.3l_resid 
ggplot2::ggplot(data = ml_meta_model.3l_resid) + geom_histogram(mapping = aes(x= resid),binwidth = 0.06, alpha = 0.8, fill= "#589E9E", color = "#000000") + theme_minimal() 
#Normality of residuals seem fine 
qqnorm(residuals(ml_meta_model.3l))
?rma.mv
ml_meta_model.3l_resid %>% mutate(std.resid = resid/m)
ml_meta_model.3l
...
rma.mv(yi = yi, V = vi, data = filter(meta_tib, NT_type.1 == "Cardinality"), slab=paste(effect), struct = "CS", method = "REML", test = "t", 
                                      random = ~ 1 | study_id/effect)

forest(rma.mv(yi = yi, V = vi, data = filter(meta_tib, NT_type.1 == "Cardinality"), slab=paste(effect), struct = "CS", method = "REML", test = "t", 
                                      random = ~ 1 | study_id/effect))

#Conventional Nominatives
#3 effects within 1 study
rma(yi = yi, vi = vi, data = filter(meta_tib, NT_type.1 == "Conventional_Nominatives"), slab=paste(effect), struct = "CS", method = "DL", test = "t")

rma.mv(yi = yi, V = vi, data = filter(meta_tib, NT_type.1 == "Conventional_Nominatives"), slab=paste(effect), struct = "CS", method = "REML", test = "t", 
                                      random = ~ 1 | effect)
forest(rma.mv(yi = yi, V = vi, data = filter(meta_tib, NT_type.1 == "Conventional_Nominatives"), slab=paste(effect), struct = "CS", method = "REML", test = "t", 
                                      random = ~ 1 | effect)) 

rma.mv(yi = yi, V = vi, data = meta_tib, slab=paste(effect), struct = "CS", method = "REML", test = "t", 
                                      random = ~ 1 | study_id/effect) %>% forest()


#Calculation
#2 effects within 1 study
rma(yi = yi, vi = vi, data = filter(meta_tib, NT_type.1 == "Calculation"), slab=paste(effect), struct = "CS", method = "DL", test = "t")
rma.mv(yi = yi, V = vi, data = filter(meta_tib, NT_type.1 == "Calculation"), slab=paste(effect), struct = "CS", method = "REML", test = "t", random = ~1 | effect)
rma.mv(yi = yi, V = vi, data = filter(meta_tib, NT_type.1 == "Calculation"), slab=paste(effect), struct = "CS", method = "REML", test = "t", random = ~1 | effect) %>% forest()



#RELEVELLING - Similar results, but why are the parameters and pvalues different
#----------------------------------------------------------------------------------------------------
meta_tib_NT_type <- select(meta_tib, effect, study_id, NT_type.1, yi, vi, n ) %>% mutate(NT_type.1 = fct_relevel(NT_type.1, "Calculation", "Cardinality", "Conventional_Nominatives", "Overall", "Number_Identification", "Ordering", "Quantity_Comparison", "Counting"))
levels(meta_tib_NT_type$NT_type.1)


#Setting contrast weights (deviation coding)
calculationNT_vs_mean2 <-                c(0.5, 0, 0, 0, 0, 0, 0, -0.5)
cardinalityNT_vs_mean2 <-                c(0, 0.5, 0, 0, 0, 0, 0, -0.5)
conventional_nominativesNT.1_vs_mean2 <- c(0, 0, 0.5, 0, 0, 0, 0, -0.5)
overallNT_vs_mean2 <-                    c(0, 0, 0, 0.5, 0, 0, 0, -0.5)
number_identificationNT_vs_mean2 <-      c(0, 0, 0, 0, 0.5, 0, 0, -0.5)
orderingNT_vs_mean2 <-                   c(0, 0, 0, 0, 0, 0.5, 0, -0.5)
quantity_comparisonNT_vs_mean2 <-        c(0, 0, 0, 0, 0, 0, 0.5, -0.5)

contrasts(meta_tib_NT_type$NT_type.1) <- cbind(
  calculationNT_vs_mean2, 
  cardinalityNT_vs_mean2,
  conventional_nominativesNT.1_vs_mean2,
  overallNT_vs_mean2,
  number_identificationNT_vs_mean2,
  orderingNT_vs_mean2,
  quantity_comparisonNT_vs_mean2)

rma.mv(yi = yi, V = vi, data = meta_tib_NT_type , struct = "CS", method = "REML", test = "t",
                                      mods = ~ NT_type.1, 
                                      random = ~ 1 | study_id/effect)
...
#RESULTS 
#--- 1 outlier (casey and boonen_et_al.2011)

#Outlier
ml_meta_model.3l_mod_MA_context.outlier.b1 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ MA_context, random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 1)
ml_meta_model.3l_mod_MA_context.outlier.b2 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ MA_context, random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 2)
ml_meta_model.3l_mod_MA_context.outlier.b3 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ MA_context, random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 3)
ml_meta_model.3l_mod_MA_context.outlier.b4 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ MA_context, random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 4)

#beta 1 outliers
ml_meta_model.3l_mod_MA_context.outlier.b1 %>% filter(cooks.d >= 1 | DFBETAS >= 1)
#Filtering studies that have cooks.d or DFBETAS >= 1
filter(ml_meta_model.3l_mod_MA_context.outlier.b1, cooks.d >= 1 | DFBETAS >= 1)

ml_meta_model.3l_mod_MA_context.outlier.b2
filter(ml_meta_model.3l_mod_MA_context.outlier.b2, cooks.d >= 1 | DFBETAS >= 1)

ml_meta_model.3l_mod_MA_context.outlier.b3
filter(ml_meta_model.3l_mod_MA_context.outlier.b3, cooks.d >= 1 | DFBETAS >= 1)


ml_meta_model.3l_mod_MA_context.outlier.b4
filter(ml_meta_model.3l_mod_MA_context.outlier.b4, cooks.d >= 1 | DFBETAS >= 1)



rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML", mods = ~ MA_context, random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 1)
...
rma.mv(yi = yi, V = vi, data = filter(meta_tib, MA_context == "Lab"), slab=paste(effect), struct = "CS", method = "REML", test = "t", 
                                      random = ~ 1 | study_id/effect)
...
rma.mv(yi = yi, V = vi, data = filter(meta_tib, MA_context == "School"), slab=paste(effect), struct = "CS", method = "REML", test = "t", 
                                      random = ~ 1 | study_id/effect)
...
#RELEVELLING (Exactly the same results; don't need to care about this)
#----------------------------------------------------------------------------------------------------
meta_tib_MA_context <- select(meta_tib, effect, study_id, MA_context, yi, vi, n ) %>% mutate(MA_context = fct_relevel(MA_context, "Home", "Lab", "School", "Unknown", "Museum"))
levels(meta_tib_MA_context$MA_context)


#Setting contrast weights (deviation coding)
home_vs_mean2 <-   c(0.5, 0, 0, 0, -0.5)
lab_vs_mean2 <-    c(0, 0.5, 0, 0, -0.5)
school_vs_mean2 <- c(0, 0, 0.5, 0, -0.5)
unknown_vs_mean2 <- c(0, 0, 0, 0.5, -0.5)
meta_tib %>% view()

contrasts(meta_tib_MA_context$MA_context) <- cbind(
  home_vs_mean2,
  lab_vs_mean2,
  school_vs_mean2,
  unknown_vs_mean2)

rma.mv(yi = yi, V = vi, data = meta_tib_MA_context, struct = "CS", method = "REML", test = "t",
                                      mods = ~ MA_context, 
                                      random = ~ 1 | study_id/effect)
...
#High mothers
rma.mv(yi = yi, V = vi, data = filter(meta_tib_mod_mothers, mothers_cat == "high"), slab=paste(effect), struct = "CS", method = "REML", test = "t", 
                                      random = ~ 1 | study_id/effect)

rma.mv(yi = yi, V = vi, data = filter(meta_tib_mod_mothers, mothers_cat == "high"), slab=paste(effect), struct = "CS", method = "REML", test = "t", 
                                      random = ~ 1 | study_id/effect) %>% forest()

rma.mv(yi = yi, V = vi, data = filter(meta_tib_mod_mothers, mothers_cat == "low"), slab=paste(effect), struct = "CS", method = "REML", test = "t", 
                                      random = ~ 1 | study_id/effect) %>% funnel()
#Funnel plot shows there might be some publication bias 
...
meta_tib %>% group_by(MA_measure) %>% summarise(n = n())
...
#DFBETA
#Level 2
dfbetas(ml_meta_model.3l_mod_MA_measure, progbar=FALSE,reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$effect) %>% rownames_to_column(var = "effect_id") %>% filter(abs(MA_measurequantity_comparisonMA_vs_mean)>1)
#DFBETAS RESULTS
#-- Intercept: All DFBETAS are below 1
#-- MA_measurecalculationMA_vs_mean: There are 2 outliers (mutaf a and mutaf b)
#-- MA_measurecardinalityMA_vs_mean: There are 2 outliers (levine_et_al.2010a and ramani_et_al.2015g)
#-- MA_measurecountingMA_vs_mean: All DFBETAS are below 1
#-- MA_measurenumber_identificationMA_vs_mean: All DFBETAS are below 1
#-- MA_measurenumber_knowledgeMA_vs_mean: There are 2 outliers (gibson_et_al.2020a and gibson_et_al.2020b)
#-- MA_measurenumber_line_estimation_MA_vs_mean: All DFBETAS are below 1
#-- MA_measurequantity_comparisonMA_vs_mean: All DFBETAS are below 1


#Level 3
dfbetas(ml_meta_model.3l_mod_MA_measure, progbar=FALSE,reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$study_id) %>% rownames_to_column(var = "study_id") %>% filter(abs(MA_measurequantity_comparisonMA_vs_mean)>1)
#DFBETAS RESULTS
#-- Intercept: All DFBETAS are below 1
#-- MA_measurecalculationMA_vs_mean: All DFBETAS are below 1
#-- MA_measurecardinalityMA_vs_mean: There are 2 outliers (levine_et_al.2010a and ramani_et_al.2015g)
#-- MA_measurecountingMA_vs_mean: There is 1 outlier (ramani_et_al.2015)
#-- MA_measurenumber_identificationMA_vs_mean: All DFBETAS are below 1
#-- MA_measurenumber_knowledgeMA_vs_mean: All DFBETAS are below 1 
#-- MA_measurenumber_line_estimation_MA_vs_mean: There are 2 outliers (ramani_et_al.2015 and zippert_et_al.2019)
#-- MA_measurequantity_comparisonMA_vs_mean: All DFBETAS are below 1


#Outlier 
#Beta 1
ml_meta_model.3l_mod_MA_measure.outlier.b1 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML",
                                      mods = ~ MA_measure, 
                                      random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 1)

filter(ml_meta_model.3l_mod_MA_measure.outlier.b1, cooks.d >= 1 | abs(DFBETAS) >= 1)

#Beta 1
ml_meta_model.3l_mod_MA_measure.outlier.b2 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML",
                                      mods = ~ MA_measure, 
                                      random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 2)

filter(ml_meta_model.3l_mod_MA_measure.outlier.b2, cooks.d >= 1 | abs(DFBETAS) >= 1)

#Beta 3
ml_meta_model.3l_mod_MA_measure.outlier.b3 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML",
                                      mods = ~ MA_measure, 
                                      random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 3)

filter(ml_meta_model.3l_mod_MA_measure.outlier.b3, cooks.d >= 1 | abs(DFBETAS) >= 1)

#Beta 4
ml_meta_model.3l_mod_MA_measure.outlier.b4 <- rma.mv.outlier(data = meta_tib, struct = "CS", method = "REML",
                                      mods = ~ MA_measure, 
                                      random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 4)

filter(ml_meta_model.3l_mod_MA_measure.outlier.b4, cooks.d >= 1 | abs(DFBETAS) >= 1)
...
#----------------------------------------------------------------------------------------------------
#Checking the levels for MA_measure
meta_tib_MA_measure <- meta_tib %>% select(effect, study_id, MA_measure,  yi, vi, n) %>% mutate(MA_measure = forcats::fct_relevel(MA_measure, "Calculation", "Cardinality", "Counting", "Number_Identification", "Overall", "Number_Line_Estimation", "Quantity_Comparison", "Number_Knowledge"))
meta_tib_MA_measure 
levels(meta_tib_MA_measure$MA_measure)

meta_tib %>% group_by(MA_measure) %>% summarise(n = n())

#Setting contrast weights (deviation coding)
calculationMA_vs_mean2 <-             c(0.5, 0, 0, 0, 0, 0, 0, -0.5)
cardinalityMA_vs_mean2 <-             c(0, 0.5, 0, 0, 0, 0, 0, -0.5)
countingMA_vs_mean2 <-                c(0, 0, 0.5, 0, 0, 0, 0, -0.5)
number_identificationMA_vs_mean2 <-   c(0, 0, 0, 0.5, 0, 0, 0, -0.5)
overallMA_vs_mean2 <-                 c(0, 0, 0, 0, 0.5, 0, 0, -0.5)
number_line_estimation_MA_vs_mean2 <- c(0, 0, 0, 0, 0, 0.5, 0, -0.5)
quantity_comparisonMA_vs_mean2 <-     c(0, 0, 0, 0, 0, 0, 0.5, -0.5)

contrasts(meta_tib_MA_measure$MA_measure) <- cbind(
  calculationMA_vs_mean2, 
  cardinalityMA_vs_mean2,
  countingMA_vs_mean2,
  number_identificationMA_vs_mean2, 
  overallMA_vs_mean2,
  number_line_estimation_MA_vs_mean2,
  quantity_comparisonMA_vs_mean2)

rma.mv(yi = yi, V = vi, data = meta_tib_MA_measure, struct = "CS", method = "REML", test = "t",
                                      mods = ~ MA_measure, 
                                      random = ~ 1 | study_id/effect)
...
weightr::weightfunct(effect = meta_tib$yi, v = meta_tib$vi)
...
#Just checking (Internet knowledge is wrong)
...
leave1out(re_meta_model)

nrow(meta_tib)
meta_test.3l(model = ml_meta_model.3l)

ml_meta_model.3l$version



rowid_to_column(rename(as.data.frame(cbind(ml_meta_model.3l$yi, ml_meta_model.3l$vi)), yi = V1, vi = V2), var = "effect_no1")





rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "REML", test = "t", 
                                      random = ~1 | study_id/effect, mods = )

ml_meta_model.3l$
meta_tib %>% rowid_to_column(var = "effect_no1")
