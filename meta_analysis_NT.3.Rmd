...
#Fixed effect SE = 0.0153
#Funnel plot
funnel(re_meta_model)

reporter(re_meta_model)
reporter(rma(yi = yi, vi = vi, data = meta_tib, method  = "FE"))
sqrt(0.0216 + (0.0153^2))

#Prediction
predict(re_meta_model, transf = transf.ztor, digits = 5)

#Gosh Plot 
plot(gosh(re_meta_model, subset = 1000), out = 10)
...
#Comparing between the 2-level with the 3-level model
fitstats(ml_meta_model.2l, ml_meta_model.3l)
#RESULTS 
#-- The 3-level model has better fit than the 2-level model 
#CONCLUSION 
#-- The 3-level model is selected 



#However, level 3 variance is negligible, I will create a second model without the level 3, thus reducing the model to 2 levels (Level 1: Sampling variance, Level 2 = Effect level)
#2-level meta-analysis
ml_meta_model.2lb <- rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "ML", test = "t",
                        random = ~ 1 | effect)
ml_meta_model.2lb %>% summary()
ml_meta_model.2lb

#Comparing models
fitstats(ml_meta_model.3l, ml_meta_model.2lb)

fitstats(ml_meta_model.3l, ml_meta_model.3lb)
ml_meta_model.3l




#Outlier analysis for the 2-level model (RESULTS: NO OUTLIERS)
#2-level model
#Cook's d at level 1 
ml_meta_model.2lb_cooks.d_lev1 <- cooks.distance(ml_meta_model.2lb, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
ml_meta_model.2lb_cooks.d_lev1 
#Cook's d plot
plot(cooks.distance(ml_meta_model.2lb, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1), type="o", pch=19, xlab="Observed Outcome", ylab="Cook's Distance")
#RESULTS 
#-- All Cook's distances are well below 1

#Cook's d at level 2 (The Effect level)
ml_meta_model.2lb_cooks.d_lev2 <- cooks.distance(ml_meta_model.2lb, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$effect) %>% tibble() %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
ml_meta_model.2lb_cooks.d_lev2 
plot(cooks.distance(ml_meta_model.2lb, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$effect), type="o", pch=19, xlab="Observed Outcome", ylab="Cook's Distance")
#RESULTS 
#-- All Cook's distances are well below 1
...
#3-level model
#Cook's d at level 1 
ml_meta_model.3l_cooks.d_lev1 <- cooks.distance(ml_meta_model.3l, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
ml_meta_model.3l_cooks.d_lev1
plot(cooks.distance(ml_meta_model.3l, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1), type="o", pch=19, xlab="Observed Outcome", ylab="Cook's Distance")
#RESULTS 
#-- All Cook's distances are well below 1

#Cook's d at level 2 (The Effect level)
ml_meta_model.3l_cooks.d_lev2 <- cooks.distance(ml_meta_model.2lb, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$effect) %>% tibble() %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
...
ml_meta_model.3l_cooks.d_lev3 <- cooks.distance(ml_meta_model.2lb, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$study_id) %>% tibble() %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
ml_meta_model.3l_cooks.d_lev3 
ml_meta_model.3l_cooks.d_lev2 
plot(cooks.distance(ml_meta_model.2lb, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$study_id), type="o", pch=19, xlab="Observed Outcome", ylab="Cook's Distance")
#RESULTS 
#-- All Cook's distances are well below 1
...
#CONCLUSION
#2 MODDELS ARE SELECTED AS WORKING MODELS 
ml_meta_model.3l
ml_meta_model.2lb

```
```{r visualising_the_models}
#Forest plots
forest(ml_meta_model.3l, addpred = T, header = T)
forest(ml_meta_model.2lb, addpred = T, header = T)
...
#2-level model b
ml_meta_model.2lb_mod_main_language <- rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "ML", test = "t",
                                          mods = ~ main_language,
                                          random = ~ 1 | effect)
ml_meta_model.2lb_mod_main_language
...
#2-level model
ml_meta_model.2l_mod_main_language_cooks.d <- cooks.distance(ml_meta_model.2l_mod_main_language, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, main_language), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) %>% arrange(desc(cooks.d))
ml_meta_model.2l_mod_main_language_cooks.d  
#RESULTS
#There are 2 outliers 


#3-level model
ml_meta_model.3l_mod_main_language_cooks.d <- cooks.distance(ml_meta_model.3l_mod_main_language, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, main_language), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) %>% arrange(desc(cooks.d))
ml_meta_model.3l_mod_main_language_cooks.d  
#RESULTS
#No outlier (same as 2-level model b)

#2-level model b
ml_meta_model.2lb_mod_main_language_cooks.d <- cooks.distance(ml_meta_model.2lb_mod_main_language, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, main_language), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) %>% arrange(desc(cooks.d))
ml_meta_model.2lb_mod_main_language_cooks.d  
#RESULTS
#No outlier (same as 2-level model b)
...
#DFBETA
#2-level model
dfbetas(ml_meta_model.2l_mod_main_language, progbar=FALSE,reestimate=TRUE, parallel="no", ncpus=1) %>% 
...
#DFBETAS RESULTS
#-- Biggest DFBETA was boonen_et_al.2011d (1.126)

#3-level model
dfbetas(ml_meta_model.3l_mod_main_language, progbar=FALSE,reestimate=TRUE, parallel="no", ncpus=1) %>% 
  add_column(select(drop_na(data = meta_tib, main_language), effect)) %>% 
  select(effect, intrcpt, main_language1) %>% 
  mutate(absDFBETAS = abs(main_language1)) %>% 
  arrange(desc(absDFBETAS))
...
#2-level model b
dfbetas(ml_meta_model.2lb_mod_main_language, progbar=FALSE,reestimate=TRUE, parallel="no", ncpus=1) %>% 
  add_column(select(drop_na(data = meta_tib, main_language), effect)) %>% 
  select(effect, intrcpt, main_language1) %>% 
  mutate(absDFBETAS = abs(main_language1)) %>% 
  arrange(desc(absDFBETAS))
...
#Outlier diagnostics for the 2-level model
#Cook's d 
#Cook's d at level 1
ml_meta_model.2l_mod_NT_type.1_cooks.d_lev1 <- cooks.distance(ml_meta_model.2l_mod_NT_type.1, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, NT_type.1), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) %>% arrange(desc(cooks.d))
ml_meta_model.2l_mod_NT_type.1_cooks.d_lev1
#RESULTS 
#--- All Cook's d are well below 1

#Cook's d at level 2
ml_meta_model.2l_mod_NT_type.1_cooks.d_lev2 <- cooks.distance(ml_meta_model.2l_mod_NT_type.1, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$study_id) %>% tibble() %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
ml_meta_model.2l_mod_NT_type.1_cooks.d_lev2
#RESULTS 
#--- Levine's study has a huge Cook's d
...
#Cook's d 
#Cook's d at level 1
ml_meta_model.3l_mod_NT_type.1_cooks.d_lev1 <- cooks.distance(ml_meta_model.3l_mod_NT_type.1, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, NT_type.1), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) %>% arrange(desc(cooks.d))
ml_meta_model.3l_mod_NT_type.1_cooks.d_lev1
#RESULTS 
#--- All Cook's d are well below 1

#Cook's d at level 2
ml_meta_model.3l_mod_NT_type.1_cooks.d_lev2 <- cooks.distance(ml_meta_model.3l_mod_NT_type.1, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$effect) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, NT_type.1), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) %>% arrange(desc(cooks.d))
ml_meta_model.3l_mod_NT_type.1_cooks.d_lev2
#RESULTS 
#--- All Cook's d are well below 1

#Cook's d at level 3
ml_meta_model.3l_mod_NT_type.1_cooks.d_lev3 <- cooks.distance(ml_meta_model.3l_mod_NT_type.1, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$study_id) %>% tibble() %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
...
#2-level model b
ml_meta_model.2lb_mod_NT_type.1 <- rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "REML", test = "t",
                                      mods = ~ NT_type.1, 
                                      random = ~ 1 | effect)
ml_meta_model.2lb_mod_NT_type.1  
#SIGNIFICANT RESULTS
#INTERCEPT = 	0.0462
#NT that have significantly greater effect 
...
#Outlier diagnostics for the 2-level model b
#Cook's d 
#Cook's d at level 1
ml_meta_model.2lb_mod_NT_type.1_cooks.d_lev1 <- cooks.distance(ml_meta_model.2lb_mod_NT_type.1, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, NT_type.1), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) %>% arrange(desc(cooks.d))
ml_meta_model.2lb_mod_NT_type.1_cooks.d_lev1
#RESULTS 
#--- All Cook's d are well below 1

#Cook's d at level 2
ml_meta_model.2lb_mod_NT_type.1_cooks.d_lev2 <- cooks.distance(ml_meta_model.2lb_mod_NT_type.1, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$effect) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, NT_type.1), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) %>% arrange(desc(cooks.d))
ml_meta_model.2lb_mod_NT_type.1_cooks.d_lev2
#RESULTS 
#--- All Cook's d are well below 1
...
#2-level model
ml_meta_model.2l_mod_MA_context <- rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "REML", test = "t",
                                       mods = ~ MA_context, 
                                       random = ~ 1 | study_id)
ml_meta_model.2l_mod_MA_context 
#RESULTS 
#Measuring MA in a Lab has a greater effect (b = 0.3437, p = 0.0010)
#Measuring MA at home has a lower effect (b = -0.3639, p = 0.0419)


#Outlier diagnostics for the 2-level model
#Cook's d 
#Cook's d at level 1
ml_meta_model.2l_mod_MA_context_cooks.d_lev1 <- cooks.distance(ml_meta_model.2l_mod_MA_context, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, MA_context), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) %>% arrange(desc(cooks.d))
ml_meta_model.2l_mod_MA_context_cooks.d_lev1
#RESULTS 
#--- 3 outliers

#Cook's d at level 2
ml_meta_model.2l_mod_MA_context_cooks.d_lev2 <- cooks.distance(ml_meta_model.2l_mod_MA_context, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$study_id) %>% tibble() %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
ml_meta_model.2l_mod_MA_context_cooks.d_lev2
#RESULTS 
#--- 1 outlier
...
#Cook's d 
#Cook's d at level 1
ml_meta_model.3l_mod_MA_context_cooks.d_lev1 <- cooks.distance(ml_meta_model.3l_mod_MA_context, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, MA_context), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) %>% arrange(desc(cooks.d))
ml_meta_model.3l_mod_MA_context_cooks.d_lev1 
#RESULTS 
#--- 1 outlier

#Cook's d at level 2
ml_meta_model.3l_mod_MA_context_cooks.d_lev2 <- cooks.distance(ml_meta_model.2l_mod_MA_context, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$effect) %>% tibble() %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
ml_meta_model.3l_mod_MA_context_cooks.d_lev2
#RESULTS 
#--- 3 outliers

#Cook's d at level 3
ml_meta_model.3l_mod_MA_context_cooks.d_lev3 <- cooks.distance(ml_meta_model.2l_mod_MA_context, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$study_id) %>% tibble() %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
...
#2-level model b
ml_meta_model.2lb_mod_MA_context <- rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "REML", test = "t",
                                       mods = ~ MA_context, 
                                       random = ~ 1 | effect)
ml_meta_model.2lb_mod_MA_context 
#RESULTS 
#Measuring MA in a Lab has a greater effect (b = 0.3432, p = 0.0178)

#Outlier diagnostics for the 2-level model b
#Cook's d 
#Cook's d at level 2
ml_meta_model.2lb_mod_MA_context_cooks.d_lev2 <- cooks.distance(ml_meta_model.2lb_mod_MA_context, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$effect) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, MA_context), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) %>% arrange(desc(cooks.d))
ml_meta_model.2lb_mod_MA_context_cooks.d_lev2 
#RESULTS 
#--- 2 outliers
...
#Moderator analysis for mothers (treating mothers as a continuous variable)
#2-level Regression
ml_meta_model.2l_mod_mothers_cont <- rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "REML", test = "t",
                                    mods = ~ mothers, 
                                    random = ~ 1 | study_id)
ml_meta_model.2l_mod_mothers_cont
#RESULTS
#The proportion of mothers significantly enhanced the effect NT on MA


#----------------------------------------------------------------------------------------------------
#Moderator analysis for mothers (treating mothers as a categorical variable)(High vs Low, cut-off defined by the mean)
...
#----------------------------------------------------------------------------------------------------
#2-level model b
ml_meta_model.2lb_mod_mothers_cat <- rma.mv(yi = yi, V = vi, data = meta_tib_mod_mothers, struct = "CS", method = "REML", test = "t",
                                    mods = ~ mothers_cat, 
                                    random = ~ 1 | effect)
ml_meta_model.2lb_mod_mothers_cat 
#RESULTS
#Mothers was significant (p = 0.0424)
regplot(ml_meta_model.2lb_mod_mothers_cat, xlab = "Proportion of Mothers")
```
...
#Cook's d
#Cook's d plot using plot
plot(cooks.distance(ml_meta_model.3l_mod_child_age_NT_mid, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1), type="o", pch=19, xlab="Observed Outcome", ylab="Cook's Distance")
#Cook's d RESULTS 
#-- All Cook's d were well below 1

#DFBETA
dfbetas(ml_meta_model.3l_mod_child_age_NT_mid, progbar=FALSE,reestimate=TRUE, parallel="no", ncpus=1) %>% 
  add_column(select(drop_na(data = meta_tib, child_age_NT_mid), effect)) %>% 
  select(effect, intrcpt, child_age_NT_mid) %>% 
  mutate(absDFBETAS = abs(child_age_NT_mid)) %>% 
  arrange(desc(absDFBETAS))
#DFBETAS RESULTS
#-- Biggest absolute DFBETA was levine_et_al.2010 (0.67)
...
#----------------------------------------------------------------------------------------------------
#2-level model b
ml_meta_model.2lb_mod_child_age_NT_mid <- rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "REML", test = "t",
                                                mods = ~ child_age_NT_mid, 
                                                random = ~ 1 | effect)
ml_meta_model.2lb_mod_child_age_NT_mid
#RESULTS 
#--- Child age during NT has a significant effect (p = 0.0149)
regplot(ml_meta_model.2lb_mod_child_age_NT_mid, xlab = "Child Age During Number Talk (Days)")
...
#Outlier Analysis
#Cook's d
child_age_NT_mid_mod_cooks.d <- cooks.distance(ml_meta_model_mod_child_age_NT_mid.M1, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, child_age_NT_mid), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(effect, Effect_no, cooks.d) 
child_age_NT_mid_mod_cooks.d %>% filter(cooks.d > 0.5)
#Cook's d plot using ggplot
ggplot(data = child_age_NT_mid_mod_cooks.d) + 
  geom_col(mapping = aes(x = Effect_no, y = cooks.d),  width = 0.3, fill = "black")  + 
...
#Cook's d plot using plot
plot(cooks.distance(ml_meta_model_mod_child_age_NT_mid.M1, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1), type="o", pch=19, xlab="Observed Outcome", ylab="Cook's Distance")
#Cook's d RESULTS 
#-- The biggest Cook's d was levine_et_al.2010a (0.85)

#DFBETA
dfbetas(ml_meta_model_mod_child_age_NT_mid.M1, progbar=FALSE,reestimate=TRUE, parallel="no", ncpus=1) %>% 
  add_column(select(drop_na(data = meta_tib, child_age_NT_mid), effect)) %>% 
  select(effect, intrcpt, child_age_NT_mid) %>% 
  mutate(absDFBETAS = abs(child_age_NT_mid)) %>% 
  arrange(desc(absDFBETAS))
#DFBETAS RESULTS
#-- Biggest DFBETA was levine_et_al.2010 (0.89)

# Hatvalues (I have no idea what this is)
hatvalues(ml_meta_model_mod_child_age_NT_mid.M1, type = "diagonal")
...
cooks.distance(ml_meta_model.3l_mod_NT_context) %>% tibble() %>% arrange(desc(.))
#RESULTS 
#All Cook's distances are below 0

#----------------------------------------------------------------------------------------------------
#2-level model b
ml_meta_model.2lb_mod_NT_context <- rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "REML", test = "t",
                                       mods = ~ NT_context, 
                                       random = ~ 1 | effect)
ml_meta_model.2lb_mod_NT_context
...
cooks.distance(ml_meta_model.3l_mod_MA_measure) %>% tibble() %>% arrange(desc(.))
#RESULTS 
#There are 6 outliers 

#----------------------------------------------------------------------------------------------------
#2-level model b
ml_meta_model.2lb_mod_MA_measure <- rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "REML", test = "t",
                                      mods = ~ MA_measure, 
                                      random = ~ 1 | effect)
ml_meta_model.2lb_mod_MA_measure
#RESULTS 
#No significant results 
...
plot(lm(snd ~ inv.se, data = meta_tib2))
