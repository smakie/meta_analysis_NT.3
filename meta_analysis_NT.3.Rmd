...
######################################################################
#This is replaced by rma.mv.outlier
rma.mv.outlier.old4 <- function(data, struct = "CS", method = "REML", random = NULL, mods = NULL, b = 0, cluster){
  data.gojira <- data
  c <- cluster
...
######################################################################
#This is the current one, it handles robust methods as well
rma.mv.influence <- function(data, struct = "CS", method = "REML", random = NULL, mods = NULL, b = 0, cluster, robust = FALSE, robust.cluster, adjust = FALSE){
  data.gojira <- data
  c1 <- cluster
  c <- unlist(as.list(array(data[c(c1)])))
  data.gojira.2 <- mutate(data.gojira, cluster.gojira = as.factor(c))
  data.gojira.2$cluster.gojira2 <- group_indices(group_by(data.gojira.2, cluster.gojira), cluster.gojira)
  if(isFALSE(robust)){
     model0 <- rma.mv(yi = yi, V = vi, data = data.gojira, struct = struct, method = method, test = "t", 
                                      random = random, mods = mods)
       leveldetermine1 <- select(mutate(rowid_to_column(rename(as.tibble(rev(unlist(strsplit(str_trim(unlist(strsplit(as.character(model0$random), "[|]"))[2], side = c("both", "left", "right")), "[/]")))), cluster = value), var = "id"), level = id + 1), level, cluster)
       leveldetermine2 <- unlist(as.list(array(select(filter(leveldetermine1 , cluster == c1), level))))}
    
  else if(isTRUE(robust)){
    model0.p1 <- rma.mv(yi = yi, V = vi, data = data.gojira, struct = struct, method = method, test = "t", 
                                      random = random, mods = mods) 
    model0 <- metafor::robust(model0.p1,  cluster = pull(data.gojira, robust.cluster), adjust = T)}
  cluster_unit <- c()
  leave1out <- c()
  se <- c()
  tval <- c()
  ...
  resid <- rename(as.tibble(select(as.data.frame(rstudent(model0)), z)), rstudent = z)
  cd.1 <- cooks.distance(model0, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = as.factor(c))
  cd.2 <- rename(mutate(rownames_to_column(as.data.frame(cd.1), var = "cluster_unit"), cluster_unit = as.factor(cluster_unit)), cooks.d = cd.1)
  if(isFALSE(robust)){
    dfbetas1 <- dfbetas(model0, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = as.factor(c))
  ...
  if(isFALSE(robust)){
    if(is.null(mods)){
        if(leveldetermine2 == 2){
          return(select(mutate(add_column(full_join(cluster_unit_col2, full_join(full_join(mutate(mutate(as.data.frame(cbind(cluster_unit, leave1out, se, tval, pval, ci.lb, ci.ub, model.aic, model.bic, model.aicc)), sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")), cluster_unit = as.factor(cluster_unit),leave1out = as.numeric(leave1out), se = as.numeric(se), tval = as.numeric(tval), pval = as.numeric(pval), ci.lb = as.numeric(ci.lb),ci.ub = as.numeric(ci.ub), model.aic = as.numeric(model.aic), model.bic = as.numeric(model.bic),  model.aicc = as.numeric(model.aicc)), cd.2, by = "cluster_unit"), dfbetas2, by = "cluster_unit"), by = "cluster_unit"), resid), cooks.d = as.numeric(cooks.d), DFBETAS = as.numeric(DFBETAS)), cluster_unit, rstudent, cooks.d, DFBETAS, leave1out, se, tval, pval, ci.lb, ci.ub, sig, model.aic, model.bic, model.aicc))}
        else if(leveldetermine2 != 2){
          return(select(mutate(full_join(cluster_unit_col2, full_join(full_join(mutate(mutate(as.data.frame(cbind(cluster_unit, leave1out, se, tval, pval, ci.lb, ci.ub, model.aic, model.bic, model.aicc)), sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")), cluster_unit = as.factor(cluster_unit),leave1out = as.numeric(leave1out), se = as.numeric(se), tval = as.numeric(tval), pval = as.numeric(pval), ci.lb = as.numeric(ci.lb),ci.ub = as.numeric(ci.ub), model.aic = as.numeric(model.aic), model.bic = as.numeric(model.bic),  model.aicc = as.numeric(model.aicc)), cd.2, by = "cluster_unit"), dfbetas2, by = "cluster_unit"), by = "cluster_unit"), cooks.d = as.numeric(cooks.d), DFBETAS = as.numeric(DFBETAS)), cluster_unit, cooks.d, DFBETAS, leave1out, se, tval, pval, ci.lb, ci.ub, sig, model.aic, model.bic, model.aicc))}
    if (!is.null(mods)) {
      if(leveldetermine2 == 2){
        return(select(mutate(add_column(full_join(cluster_unit_col2, full_join(full_join(mutate(mutate(as.data.frame(cbind(cluster_unit, leave1out, se, tval, pval, ci.lb, ci.ub, model.F, model.F.p, model.aic, model.bic, model.aicc)), sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")), cluster_unit = as.factor(cluster_unit),leave1out = as.numeric(leave1out), se = as.numeric(se), tval = as.numeric(tval), pval = as.numeric(pval), ci.lb = as.numeric(ci.lb),ci.ub = as.numeric(ci.ub), model.F = as.numeric(model.F), model.F.p = as.numeric(model.F.p), model.aic = as.numeric(model.aic), model.bic = as.numeric(model.bic),  model.aicc = as.numeric(model.aicc)), cd.2, by = "cluster_unit"), dfbetas2, by = "cluster_unit"), by = "cluster_unit"), resid), cooks.d = as.numeric(cooks.d), DFBETAS = as.numeric(DFBETAS)), cluster_unit, rstudent, cooks.d, DFBETAS, leave1out, se, tval, pval, ci.lb, ci.ub, sig, model.F, model.F.p, model.aic, model.bic, model.aicc))}
      else if(leveldetermine2 != 2){
        return(select(mutate(full_join(cluster_unit_col2, full_join(full_join(mutate(mutate(as.data.frame(cbind(cluster_unit, leave1out, se, tval, pval, ci.lb, ci.ub, model.F, model.F.p, model.aic, model.bic, model.aicc)), sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")), cluster_unit = as.factor(cluster_unit),leave1out = as.numeric(leave1out), se = as.numeric(se), tval = as.numeric(tval), pval = as.numeric(pval), ci.lb = as.numeric(ci.lb),ci.ub = as.numeric(ci.ub), model.F = as.numeric(model.F), model.F.p = as.numeric(model.F.p), model.aic = as.numeric(model.aic), model.bic = as.numeric(model.bic),  model.aicc = as.numeric(model.aicc)), cd.2, by = "cluster_unit"), dfbetas2, by = "cluster_unit"), by = "cluster_unit"), cooks.d = as.numeric(cooks.d), DFBETAS = as.numeric(DFBETAS)), cluster_unit, cooks.d, DFBETAS, leave1out, se, tval, pval, ci.lb, ci.ub, sig, model.F, model.F.p, model.aic, model.bic, model.aicc))}}}}
  ...
  if(isTRUE(robust)){
        if(is.null(mods)){
          if(leveldetermine2 == 2){
            return(select(mutate(add_column(full_join(cluster_unit_col2, full_join(mutate(mutate(as.data.frame(cbind(cluster_unit, leave1out, se, tval, pval, ci.lb, ci.ub, model.aic, model.bic, model.aicc)), sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")), cluster_unit = as.factor(cluster_unit),leave1out = as.numeric(leave1out), se = as.numeric(se), tval = as.numeric(tval), pval = as.numeric(pval), ci.lb = as.numeric(ci.lb),ci.ub = as.numeric(ci.ub), model.aic = as.numeric(model.aic), model.bic = as.numeric(model.bic),  model.aicc = as.numeric(model.aicc)), cd.2, by = "cluster_unit"), by = "cluster_unit"), resid), cooks.d = as.numeric(cooks.d)), cluster_unit, rstudent, cooks.d, leave1out, se, tval, pval, ci.lb, ci.ub, sig, model.aic, model.bic, model.aicc))}
            else if(leveldetermine2 != 2){
    ...
    if (!is.null(mods)) {
      if(leveldetermine2 == 2){
        return(select(mutate(add_column(full_join(cluster_unit_col2, full_join(mutate(mutate(as.data.frame(cbind(cluster_unit, leave1out, se, tval, pval, ci.lb, ci.ub, model.F, model.F.p, model.aic, model.bic, model.aicc)), sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")), cluster_unit = as.factor(cluster_unit),leave1out = as.numeric(leave1out), se = as.numeric(se), tval = as.numeric(tval), pval = as.numeric(pval), ci.lb = as.numeric(ci.lb),ci.ub = as.numeric(ci.ub), model.F = as.numeric(model.F), model.F.p = as.numeric(model.F.p), model.aic = as.numeric(model.aic), model.bic = as.numeric(model.bic),  model.aicc = as.numeric(model.aicc)), cd.2, by = "cluster_unit"), by = "cluster_unit"), resid), cooks.d = as.numeric(cooks.d)), cluster_unit, rstudent, cooks.d, leave1out, se, tval, pval, ci.lb, ci.ub, sig, model.F, model.F.p, model.aic, model.bic, model.aicc))}
      else if(leveldetermine2 != 2){
...
#resid <- rename(as.tibble(select(as.data.frame(rstudent(model0)), z)), rstudent = z)
#Both DFBETAS and rstudent do not work for rma robust models 

######################################################################
#This is the 2nd version, which only handles the normal model at level 2 cluster but also compute the rstudent
rma.mv.influence.lev2 <- function(data, struct = "CS", method = "REML", random = NULL, mods = NULL, b = 0, cluster, robust = FALSE, robust.cluster, adjust = FALSE){
  data.gojira <- data
  c <- cluster
  data.gojira.2 <- mutate(data.gojira, cluster.gojira = as.factor(c))
  data.gojira.2$cluster.gojira2 <- group_indices(group_by(data.gojira.2, cluster.gojira), cluster.gojira)
...
#Effect level
#Beta 1
ml_meta_model.3l.influence.b1.lev2 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 1, robust = F, robust.cluster = "effect", adjust = T)
ml_meta_model.3l.influence.b1.lev2 %>% filter(cooks.d >= 1)
#Based on Cook's d, there are no influential cases

#STUDY LEVEL
#Beta 1
ml_meta_model.3l.influence.b1.lev3 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 1, robust = F, robust.cluster = "effect", adjust = T)
ml_meta_model.3l.influence.b1.lev3 %>% filter(cooks.d >= 1)
...
#Effect level
#Beta 1
ml_meta_model.3l_mod_main_language.b1.lev2 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 1, robust = T, robust.cluster = "effect", adjust = T)
...
#STUDY LEVEL
#Beta 1
ml_meta_model.3l_mod_main_language.b1.lev3 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 1, robust = T, robust.cluster = "effect", adjust = T)
...
#Re-run the analysis with the effects filtered out
...
#Effect level
#Beta 1
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b1 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 1, robust = T, robust.cluster = "effect",adjust = T)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b1 %>% filter(cooks.d >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b1 
#Based on Cook's d and DFBETAS, There are no outliers and influential cases 

#Beta 2
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b2 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 2, robust = T, robust.cluster = "effect",adjust = T)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b2 
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b2  %>% filter(cooks.d >= 1)
#Based on Cook's d and DFBETAS, There are no outliers and influential cases 

#Beta 3
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b3 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 3, robust = T, robust.cluster = "effect",adjust = T)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b3 %>% filter(cooks.d >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b3
#Based on Cook's d and DFBETAS, There are no outliers and influential cases 

#Beta 4
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b4 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 4, robust = T, robust.cluster = "effect",adjust = T)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b4 %>% filter(cooks.d >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b4
#Based on Cook's d and DFBETAS, There are no outliers and influential cases 

#Beta 5
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b5 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 5, robust = T, robust.cluster = "effect",adjust = T)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b5 %>% filter(cooks.d >= 1 )
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b5
#Based on Cook's d and DFBETAS, There are no outliers and influential cases 

#Beta 6
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b6 <-rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 6, robust = T, robust.cluster = "effect",adjust = T)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b6 %>% filter(cooks.d >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b6
#Based on Cook's d and DFBETAS, There are no outliers and influential cases 

#Beta 7
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b7 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$effect, b = 7, robust = T, robust.cluster = "effect",adjust = T)
ml_meta_model.3l_mod_NT_type.1_outlier.lev2.b7 %>% filter(cooks.d >= 1)
...
#STUDY LEVEL
#Beta 1
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b1 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 1, robust = T, robust.cluster = "effect",adjust = T)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b1 %>% filter(cooks.d >= 1)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b1 


#Beta 2
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b2 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 2, robust = T, robust.cluster = "effect",adjust = T)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b2  %>% filter(cooks.d >= 1)
##Based on DFBETAS, there is 1 influential study (casey_et_al.2018)

#Beta 3
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b3 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 3, robust = T, robust.cluster = "effect",adjust = T)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b3 %>% filter(cooks.d >= 1)



#Beta 4
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b4 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 4, robust = T, robust.cluster = "effect",adjust = T)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b4 %>% filter(cooks.d >= 1 )



#Beta 5
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b5 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 5, robust = T, robust.cluster = "effect",adjust = T)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b5 %>% filter(cooks.d >= 1)

 

#Beta 6
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b6 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 6, robust = T, robust.cluster = "effect",adjust = T)
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b6 %>% filter(cooks.d >= 1)



#Beta 7
ml_meta_model.3l_mod_NT_type.1_outlier.lev3.b7 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ NT_type.1, random = ~1 | study_id/effect, cluster = meta_tib$study_id, b = 7, robust = T, robust.cluster = "effect",adjust = T)
...
#EFFECT LEVEL 
#Beta 1
ml_meta_model.3l_mod_MA_context.outlier.b1 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ MA_context, random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 1, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_context.outlier.b1 
ml_meta_model.3l_mod_MA_context.outlier.b1 %>% filter(cooks.d >= 1)
#Based on Cook's d, there are 2 influential cases (mutaf a and mutaf b)

#Beta 2
ml_meta_model.3l_mod_MA_context.outlier.b2 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ MA_context, random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 2, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_context.outlier.b2 
ml_meta_model.3l_mod_MA_context.outlier.b2 %>% filter(cooks.d >= 1)
#Based on Cook's d, there are 2 influential cases (mutaf a and mutaf b)

#Beta 3
ml_meta_model.3l_mod_MA_context.outlier.b3 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ MA_context, random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 3, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_context.outlier.b3 %>% filter(cooks.d >= 1)
#Based on Cook's d, there are 2 influential cases (mutaf a and mutaf b)

#Beta 4
ml_meta_model.3l_mod_MA_context.outlier.b4 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ MA_context, random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 4, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_context.outlier.b4 %>% filter(cooks.d >= 1)
#Based on Cook's d, there are 2 influential cases (mutaf a and mutaf b)


#STUDY LEVEL 
#Beta 1
ml_meta_model.3l_mod_MA_context.outlier.b1.lev3 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ MA_context, random = ~ 1 | study_id/effect, cluster = meta_tib$study_id, b = 1, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_context.outlier.b1.lev3
ml_meta_model.3l_mod_MA_context.outlier.b1.lev3 %>% filter(cooks.d >= 1)
#Based on Cook's d, there is 1 influential study (casey_et_al.2018)

#Beta 2
ml_meta_model.3l_mod_MA_context.outlier.b2.lev3 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ MA_context, random = ~ 1 | study_id/effect, cluster = meta_tib$study_id, b = 2, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_context.outlier.b2.lev3 
ml_meta_model.3l_mod_MA_context.outlier.b2.lev3 %>% filter(cooks.d >= 1)
#Based on Cook's d, there is 1 influential study (casey_et_al.2018)

#Beta 3
ml_meta_model.3l_mod_MA_context.outlier.b3.lev3 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ MA_context, random = ~ 1 | study_id/effect, cluster = meta_tib$study_id, b = 3, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_context.outlier.b3.lev3 %>% filter(cooks.d >= 1)
#Based on Cook's d, there is 1 influential study (casey_et_al.2018)

#Beta 4
ml_meta_model.3l_mod_MA_context.outlier.b4.lev3 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ MA_context, random = ~ 1 | study_id/effect, cluster = meta_tib$study_id, b = 4, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_context.outlier.b4.lev3 %>% filter(cooks.d >= 1)
#Based on Cook's d, there is 1 influential study (casey_et_al.2018)
...
#EFFECT LEVEL 
#Beta 1
ml_meta_model.3l_mod_mothers_cat.rbst.influence.b1.lev2 <- rma.mv.influence( data = meta_tib_mod_mothers, struct = "CS", method = "REML",mods = ~ mothers_cat, random = ~ 1 | study_id/effect, cluster = meta_tib_mod_mothers$effect, b = 1, robust = T, robust.cluster = "effect", adjust = T)
...
#STUDY LEVEL 
#Beta 1
ml_meta_model.3l_mod_mothers_cat.rbst.influence.b1.lev3 <- rma.mv.influence( data = meta_tib_mod_mothers, struct = "CS", method = "REML",mods = ~ mothers_cat, random = ~ 1 | study_id/effect, cluster = meta_tib_mod_mothers$study_id, b = 1, robust = T, robust.cluster = "effect", adjust = T)
...
#EFFECT LEVEL 
#Beta 1
ml_meta_model.3l_mod_child_age_NT_mid.rbst.influence.b1.lev2 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ child_age_NT_mid, random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 1, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_child_age_NT_mid.rbst.influence.b1.lev2
ml_meta_model.3l_mod_child_age_NT_mid.rbst.influence.b1.lev2 %>% filter(cooks.d >= 1)
#Based on Cook's d, there are no influential cases 

#STUDY LEVEL 
#Beta 1
ml_meta_model.3l_mod_child_age_NT_mid.rbst.influence.b1.lev3 <- rma.mv.influence(data = meta_tib, struct = "CS", method = "REML", mods = ~ child_age_NT_mid, random = ~ 1 | study_id/effect, cluster = meta_tib$study_id, b = 1, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_child_age_NT_mid.rbst.influence.b1.lev3
ml_meta_model.3l_mod_child_age_NT_mid.rbst.influence.b1.lev3 %>% filter(cooks.d >= 1)
#Based on Cook's d, there is 1 influential study (casey_et_al.2018)
...
#Bonferroni correction 
bonferroni(ml_meta_model.3l_mod_MA_measure.rbst)
#RESULTS 
...
#Beta 1
ml_meta_model.3l_mod_MA_measure.rbst.influence.b1.lev2 <- rma.mv.influence(data = meta_tib, mods = ~MA_measure, random = ~1 | study_id/effect, struct = "CS", method = "REML", cluster = meta_tib$effect, b = 1, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_measure.rbst.influence.b1.lev2
ml_meta_model.3l_mod_MA_measure.rbst.influence.b1.lev2 %>% filter(cooks.d >= 1)
#Based on Cook's d, there are 7 influential cases


#Beta 2
ml_meta_model.3l_mod_MA_measure.rbst.influence.b2.lev2 <- rma.mv.influence(data = meta_tib, mods = ~MA_measure, random = ~1 | study_id/effect, struct = "CS", method = "REML", cluster = meta_tib$effect, b = 2, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_measure.rbst.influence.b2.lev2
ml_meta_model.3l_mod_MA_measure.rbst.influence.b2.lev2 %>% filter(cooks.d >= 1)
#Based on Cook's d, there are 7 influential cases


#Beta 3
ml_meta_model.3l_mod_MA_measure.rbst.influence.b3.lev2 <- rma.mv.influence(data = meta_tib, mods = ~MA_measure, random = ~1 | study_id/effect, struct = "CS", method = "REML", cluster = meta_tib$effect, b = 3, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_measure.rbst.influence.b3.lev2
ml_meta_model.3l_mod_MA_measure.rbst.influence.b3.lev2 %>% filter(cooks.d >= 1)
#Based on Cook's d, there are 7 influential cases


#Beta 4
ml_meta_model.3l_mod_MA_measure.rbst.influence.b4.lev2 <- rma.mv.influence(data = meta_tib, mods = ~MA_measure, random = ~1 | study_id/effect, struct = "CS", method = "REML", cluster = meta_tib$effect, b = 4, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_measure.rbst.influence.b4.lev2
ml_meta_model.3l_mod_MA_measure.rbst.influence.b4.lev2 %>% filter(cooks.d >= 1)
#Based on Cook's d, there are 7 influential cases

#Beta 5
ml_meta_model.3l_mod_MA_measure.rbst.influence.b5.lev2 <- rma.mv.influence(data = meta_tib, mods = ~MA_measure, random = ~1 | study_id/effect, struct = "CS", method = "REML", cluster = meta_tib$effect, b = 5, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_measure.rbst.influence.b5.lev2
ml_meta_model.3l_mod_MA_measure.rbst.influence.b5.lev2 %>% filter(cooks.d >= 1)
#Based on Cook's d, there are 7 influential cases

#Beta 6
ml_meta_model.3l_mod_MA_measure.rbst.influence.b6.lev2 <- rma.mv.influence(data = meta_tib, mods = ~MA_measure, random = ~1 | study_id/effect, struct = "CS", method = "REML", cluster = meta_tib$effect, b = 6, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_measure.rbst.influence.b6.lev2
ml_meta_model.3l_mod_MA_measure.rbst.influence.b6.lev2 %>% filter(cooks.d >= 1)
#Based on Cook's d, there are 7 influential cases

#Beta 7
ml_meta_model.3l_mod_MA_measure.rbst.influence.b7.lev2 <- rma.mv.influence(data = meta_tib, mods = ~MA_measure, random = ~1 | study_id/effect, struct = "CS", method = "REML", cluster = meta_tib$effect, b = 7, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_measure.rbst.influence.b7.lev2
ml_meta_model.3l_mod_MA_measure.rbst.influence.b7.lev2 %>% filter(cooks.d >= 1)
#Based on Cook's d, there are 7 influential cases
...
#STUDY LEVEL 
#Beta 1
ml_meta_model.3l_mod_MA_measure.rbst.influence.b1.lev3 <- rma.mv.influence(data = meta_tib, mods = ~MA_measure, random = ~1 | study_id/effect, struct = "CS", method = "REML", cluster = meta_tib$study_id, b = 1, robust = T, robust.cluster = "effect", adjust = T)
ml_meta_model.3l_mod_MA_measure.rbst.influence.b1.lev3
ml_meta_model.3l_mod_MA_measure.rbst.influence.b1.lev3 %>% filter(cooks.d >= 1)
...
as.character(ml_meta_model.3l$random)

unlist(strsplit(as.character(ml_meta_model.3l$random), "[|]"))[2]
...
#Just checking (That youtube video??)
