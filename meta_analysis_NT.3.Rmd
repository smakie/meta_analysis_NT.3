...
rma.mv.leave1out_old <- function(data, struct = "CS", method = "REML", random, mods = NULL){
...
######################################################################
#old function - replaced by rma.mv.outlier
rma.mv.leave1out_old2 <- function(data, struct = "CS", method = "REML", random, mods = NULL, b = 0){
...
######################################################################
rma.mv.outlier <- function(data, struct = "CS", method = "REML", random, mods = NULL, b = 0, cluster){
  c <- cluster
  data1 <- rowid_to_column(data, var = "effect_no")
  model0 <- rma.mv(yi = yi, V = vi, data = data1, struct = struct, method = method, test = "t", 
                                      random = random, mods = mods)
...
#Rules for the rma.mv.outlier function 
#Effect size and variance variables in the Meta-analysis data must be yi and vi respectively 
#b - if intercept, then b = 0, if the first predictor (b1), then b = 1, and so on 
...
#Testing my function
#----------------------------------------------------------------------------------------------------
#rma.mv.leave1out.new(meta_tib, struct = "CS", method = "REML", random = ~ 1 | study_id/effect, cluster = meta_tib$effect, b = 0)
#c <- meta_tib$effect
...
#DFBETAS
#Effect level 
dfbetas(ml_meta_model.3l, cluster = meta_tib$effect, reestimate = T) %>% rownames_to_column(var = "effect") %>% mutate(absDFBETAS = abs(intrcpt)) %>% arrange(desc(absDFBETAS))
#No outliers

#Study level 
dfbetas(ml_meta_model.3l, cluster = meta_tib$study_id, reestimate = T) %>% rownames_to_column(var = "effect") %>% mutate(absDFBETAS = abs(intrcpt)) %>% arrange(desc(absDFBETAS))
#No outliers


#Leave-1-out 
ml_meta_model.3l_leave1out <- rma.mv.leave1out(meta_tib, struct = "CS",method = "REML", random = ~1 | study_id/effect)
ml_meta_model.3l_leave1out 

ml_meta_model.3l_leave1out <- rma.mv.leave1out(meta_tib, struct = "CS",method = "REML", random = ~1 | study_id/effect, cluster = meta_tib$effect)


#Validation for the leave1out function
#cooks.distance(ml_meta_model.3l, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$effect) %>% as.data.frame() %>% rename(., cooks.d = .) %>%  rownames_to_column(var = "effect") 

#dfbetas(ml_meta_model.3l, cluster = meta_tib$effect, reestimate = T) %>% rownames_to_column(var = "effect") %>% mutate(absDFBETAS = abs(intrcpt)) 
