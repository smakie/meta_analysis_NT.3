...
sandwich.influence <- function(data, struct = "CS", method = "REML", random = NULL, mods = NULL, b = 0, cluster, robust.cluster, adjust = FALSE, CHE = FALSE, r, test = "Satterthwaite"){
  ...
  for(counter in 1:max(data.gojira.2$cluster.gojira2)){
    ...
    t <- c(tval,  unlist(array(c(model[(b+1),][4]))))
...
meta_tib %>% filter(MA_measure == "Number_Line_Estimation")
...
vardist.3l(ml_meta_model.3l)

ml_meta_model.3lCHE.rbst <- coef_test(ml_meta_model.3lCHE, vcov = "CR2", cluster = meta_tib$study_id)


coef_test(ml_meta_model.3lCHE, vcov = "CR2", cluster = meta_tib$study_id)
dpplot <- dependency.sen(data = meta_tib, random = ~ 1 | study_id/effect, cluster = meta_tib$study_id, r = seq(0, 0.9, 0.01))
...
ml_meta_model.3lCHE.rbst.cs <- coef_test(ml_meta_model.3lCHE, vcov="CR2", cluster=meta_tib$study_id, test = "z") 
ml_meta_model.3lCHE.rbst.cs %>% as.list()
coef_test(ml_meta_model.3lCHE, vcov="CR2", cluster=meta_tib$study_id, test = "naive-t") %>% c()
...
robust(ml_meta_model.3lCHE, cluster = meta_tib$study_id, adjust = T)
ml_meta_model.3lCHE$call
ml_meta_model.3lCHE.influence.eff <- rma.mv.influence(random = ~1 | study_id/effect, struct = "CS", data = meta_tib, method = "REML", cluster = "effect", CHE = TRUE, r = dr, robust.cluster = "study_id")
...
rma.mv.influence(random = ~1 | study_id/effect, struct = "CS", data = meta_tib, method = "REML", cluster = "study_id", CHE = TRUE, r = dr, robust.cluster = "study_id")

ml_meta_model.3lCHE.influence %>% filter(abs(rstudent) >= 1.96 | abs(cooks.d) >= 1 | abs(DFBETAS) >= 1)

ml_meta_model.3lCHE.influence %>% filter(abs(rstudent)) %>%  dplyr::mutate(dplyr::across(where(is.numeric), ~round(., 2))) %>% kable(format = "html", caption = "Leave1out analysis", col.names = c("Effect", "Studentized residual", "Cook's Distance", "DFBETAS", "Leave1out ES", "SE", "t", "p", "CI lower", "CI upper")) %>% kable_styling(full_width = T, position = "center") %>% row_spec(row = 0, extra_css = "border-bottom: 2px solid black;")
```
...
#Beta 1
ml_meta_model.3l_mod_MA_measure.influence.b1.lev2 <- sandwich.influence(ml_meta_model.3lCHE_mod_MA_measure, random = ~1 | study_id/effect, struct = "CS", data = meta_tib, method = "REML", cluster = "effect", robust.cluster = "study_id",CHE = TRUE, r = dr, mods = ~MA_measure, b = 1)
...
#Based on Cook's d, there are 7 influential cases
#Beta 5
ml_meta_model.3l_mod_MA_measure.rbst.influence.b5.lev2 <- sandwich.influence(ml_meta_model.3lCHE_mod_MA_measure, random = ~1 | study_id/effect, struct = "CS", data = meta_tib, method = "REML", cluster = "effect", robust.cluster = "study_id",CHE = TRUE, r = dr, mods = ~MA_measure, b = 5)
#ml_meta_model.3l_mod_MA_measure.rbst.influence.b5.lev2
#Based on Cook's d, there are 7 influential cases
#Beta 6
ml_meta_model.3l_mod_MA_measure.influence.b6.lev2 <- sandwich.influence(ml_meta_model.3lCHE_mod_MA_measure, random = ~1 | study_id/effect, struct = "CS", data = meta_tib, method = "REML", cluster = "effect", robust.cluster = "study_id",CHE = TRUE, r = dr, mods = ~MA_measure, b = 6)
...
ml_meta_model.3l_mod_MA_measure.influence.b1.lev2 %>% filter(abs(rstudent) >= 1.96 | abs(cooks.d) >= 1)  
ml_meta_model.3l_mod_MA_measure.rbst.influence.b5.lev2 %>% filter(abs(rstudent) >= 1.96 | abs(cooks.d) >= 1) 

ml_meta_model.3l_mod_MA_measure.influence.b6.lev2  %>% filter(abs(rstudent) >= 1.96 | abs(cooks.d) >= 1)  
```
...
#CLUBSANDWICH 
coef_test(ml_meta_model.3lCHE_mod_MA_context, vcov = "CR2", cluster = meta_tib$study_id, test = "naive-tp")
...
#----------------------------------------------------------------------------------------------------
meta_tib_mod_arc <- meta_tib %>% mutate(mothers.arc = asin(sqrt(mothers)), female.arc = asin(sqrt(female)), white.arc = asin(sqrt(white)))

ml_meta_model.3lCHE_mod_mothers_arc <- rma.mv(yi = yi, V = v_CHE, data = meta_tib_mod_mothers.arcsine, struct = "CS", method = "REML", test = "t",
                                            mods = ~ mothers.arc, 
                                            random = ~ 1 | study_id/effect)
...
#Regression Plot
regplot(ml_meta_model.3lCHE_mod_mothers_arc , xlab = "Proportion of Mothers")
...
#CLUBSANDWICH 
coef_test(ml_meta_model.3lCHE_mod_child_age_NT_mid, vcov = "CR2", cluster=meta_tib$study_id, test = "naive-tp")
...
#Setting contrast weights 
NThome_vs_lab <-   c(1/2, -1/2, 0)
NThome_vs_school <-    c(1/2, 0, -1/2)
NTlab_vs_school <- c(0, 1/2, -1/2)

contrasts(meta_tib$NT_context) <- cbind(
NThome_vs_lab,
 NThome_vs_school,NTlab_vs_school)
...
#CLUBSANDWICH 
coef_test(ml_meta_model.3lCHE_mod_NT_context, cluster = meta_tib$study_id, vcov = "CR2", test = "naive-tp")
...
lm.beta::lm.beta(egger )
parameters::model_parameters(egger, bootstrap = T) %>% summary()
egger.boot <- bootstrap_model(egger, iterations = 10000)
egger.boot$`(Intercept)` %>% mean()
#Regression plot
...
taf.3lCHE <- trimfill(ml_meta_model.3lCHE)
