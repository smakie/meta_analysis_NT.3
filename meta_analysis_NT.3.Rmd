...
rma.mv.leave1out <- function(data, struct = "CS", method = "REML", random, mods = NULL){
  ...
  for(counter in 1:max(data1$effect_no1)){
    ...
    zval <- c(zval, modresult$zval)
    pval <- c(pval, modresult$pval)
    ci.lb <- c(ci.lb, modresult$ci.lb)
    ci.ub <- c(ci.ub, modresult$ci.ub)
  }
  return(mutate(add_column(as.data.frame(cbind(leave1out, se, zval, pval, ci.lb, ci.ub)), select(meta_tib, effect), .before = T), 
...
rma.mv.leave1out.fct <- function(data, struct = "CS", method = "REML", random, mods = NULL, dummy_var){
  data1 <- rowid_to_column(data, var = "effect_no1")
  leave1out <- c()
  se <- c()
  zval <- c()
  ...
  for(counter in 1:max(data1$effect_no1)){
    modresult <- rma.mv(yi = yi, V = vi, data = filter(data1, effect_no1 != counter), struct = struct, method = method, test = "t", 
                                      random = random, mods = mods)
    leave1out <- c(leave1out , modresult$b[dummy_var+1])
    se <- c(se, modresult$se[dummy_var+1])
    zval <- c(zval, modresult$zval[dummy_var+1])
    pval <- c(pval, modresult$pval[dummy_var+1])
    ci.lb <- c(ci.lb, modresult$ci.lb[dummy_var+1])
    ci.ub <- c(ci.ub, modresult$ci.ub[dummy_var+1])
  }
  return(mutate(add_column(as.data.frame(cbind(leave1out, se, zval, pval, ci.lb, ci.ub)), select(meta_tib, effect), .before = T), 
                sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")))
}
...
dfbetas(ml_meta_model.3l, cluster = meta_tib$effect, reestimate = F)

#Leave-1-out 
rma.mv.leave1out(meta_tib, struct = "CS", method = "REML", random = ~1 | study_id/effect) %>% mutate(estimate_change = leave1out - ml_meta_model.3l$b)
...
#Leave-1-out 
#Create function 
rma.mv.leave1out.2 <- function(data, struct = "CS", method = "REML", random, mods = NULL){
  data1 <- rowid_to_column(data, var = "effect_no1")
  leave1out <- c()
  se <- c()
  zval <- c()
  ...
  for(counter in 1:max(data1$effect_no1)){
    modresult <- rma.mv(yi = yi, V = vi, data = filter(data1, effect_no1 != counter), struct = struct, method = method, test = "t", 
                                      random = random, mods = mods)
    leave1out <- c(leave1out , modresult$b[2])
    se <- c(se, modresult$se[2])
    zval <- c(zval, modresult$zval[2])
    pval <- c(pval, modresult$pval[2])
    ci.lb <- c(ci.lb, modresult$ci.lb[2])
    ci.ub <- c(ci.ub, modresult$ci.ub[2])
  }
  return(mutate(add_column(as.data.frame(cbind(leave1out, se, zval, pval, ci.lb, ci.ub)), select(meta_tib, effect), .before = T), 
                sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")))
}

rma.mv.leave1out.2(meta_tib, struct = "CS", method = "REML", mods = ~ main_language,random = ~1 | study_id/effect) %>% mutate(estimate_change = leave1out - ml_meta_model.3l_mod_main_language$b[2])
...
#Leave 1 out
rma.mv.leave1out.fct(data = meta_tib, struct = "CS", method = "REML",
                                      mods = ~ NT_type.1, 
                                      random = ~ 1 | study_id/effect, dummy_var = 2)
