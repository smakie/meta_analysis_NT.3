...
#Cook's d at level 1
ml_meta_model.2l_cooks.d <- cooks.distance(ml_meta_model.2l, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(meta_tib, effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) %>% arrange(desc(cooks.d))
ml_meta_model.2l_cooks.d

#Cook's d at level 1 plot 
plot(cooks.distance(ml_meta_model.2l, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1), type="o", pch=19, xlab="Observed Outcome", ylab="Cook's Distance")


#Cook's d at level 2
ml_meta_model.2l_cooks.d_study.lev <- cooks.distance(ml_meta_model.2l, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$study_id) %>% tibble() %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
ml_meta_model.2l_cooks.d_study.lev

#Cook's d at level 2 plot 
plot(cooks.distance(ml_meta_model.2l, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$study_id), type="o", pch=19, xlab="Observed Outcome", ylab="Cook's Distance")
...
ml_meta_model.3l <- rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "REML", test = "t",
                        random = ~ 1 | effect_id/study_id)
ml_meta_model.3l

#Outliers analysis

#Cook's d at level 1
ml_meta_model.3l_cooks.d <- cooks.distance(ml_meta_model.3l, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(meta_tib, effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) %>% arrange(desc(cooks.d))
ml_meta_model.3l_cooks.d
...
#Cook's d at level 2
ml_meta_model.3l_cooks.d_study.lev <- cooks.distance(ml_meta_model.3l, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$study_id) %>% tibble() %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
ml_meta_model.3l_cooks.d_study.lev

#Cook's d at level 2 plot 
plot(cooks.distance(ml_meta_model.2l, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$effect_id), type="o", pch=19, xlab="Observed Outcome", ylab="Cook's Distance")



#Cook's d at level 3
ml_meta_model.3l_cooks.d_effect.lev <- cooks.distance(ml_meta_model.3l, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$effect_id) %>% tibble() %>% rename(., cooks.d = .) %>% arrange(desc(cooks.d))
ml_meta_model.3l_cooks.d_study.lev

#Cook's d at level 3 plot 
plot(cooks.distance(ml_meta_model.3l, progbar=FALSE, reestimate=TRUE, parallel="no", ncpus=1, cluster = meta_tib$study_id), type="o", pch=19, xlab="Observed Outcome", ylab="Cook's Distance")
#One study is super crazy 
...
#Clustered Robust SE
robust.rma.mv(ml_meta_model.3l, cluster = meta_tib$effect_id, clubSandwich = T)
#Results 
...
#Forest plots
forest(re_meta_model, addpred = T, header = T)
forest(ml_meta_model.2l, addpred = T, header = T)
...
#--RESULTS
# English has a greater positive effect 
...
#Outlier Analysis
#Cook's d
main_language_mod_cooks.d <- cooks.distance(ml_meta_model_mod_main_language, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, main_language), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) 
main_language_mod_cooks.d
main_language_mod_cooks.d %>% filter(cooks.d > 1)

cooks.distance(ml_meta_model_mod_main_language, progbar=FALSE,
               reestimate=TRUE, parallel="no", ncpus=1) %>% tibble() %>% rename(., cooks.d = .) %>% add_column(select(drop_na(data = meta_tib, main_language), effect)) %>% rowid_to_column(., var = "Effect_no") %>% select(Effect_no, effect, cooks.d) 
main_language_mod_cooks.d
main_language_mod_cooks.d %>% filter(cooks.d > 1)
...
#DFBETA
dfbetas(ml_meta_model_mod_main_language, progbar=FALSE,reestimate=TRUE, parallel="no", ncpus=1) %>% 
...
#DFBETAS RESULTS
#-- Biggest DFBETA was boonen_et_al.2011d (1.1264002679)

# Hatvalues 
hatvalues(ml_meta_model_mod_main_language, type = "diagonal")

1#Re-run the analysis with the effects filtered out
rma.mv(yi = yi, V = vi, data = filter(meta_tib, effect != c("boonen_et_al.2011b", "boonen_et_al.2011d")), struct = "CS", method = "ML", test = "t",
                                          mods = ~ main_language,
                                          random = ~ 1 | study_id)
...
#Measuring MA at home has a lower effect 
...
#Re-run analysis without Levin_et_al.2010a (sorry Levine)
ml_meta_model_mod_child_age_NT_mid.M2 <- rma.mv(yi = yi, V = vi, data = filter(meta_tib, effect != "levine_et_al.2010a"), struct = "CS", method = "REML", test = "t",
                                                mods = ~ child_age_NT_mid, 
                                                random = ~ 1 | study_id)
ml_meta_model_mod_child_age_NT_mid.M2

regplot(ml_meta_model_mod_child_age_NT_mid.M2, xlab = "Child Age During Number Talk (Days)")

#RESULTS 
#-- After removing Levine's study, Child_age_NT_mid is non significant (b = -0.0002, p = 0.0508)
...
ml_meta_model_mod_child_age_NT_mid.M3 <- rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "REML", test = "t",
                                                mods = ~ time_between_NT_mid_MA + child_age_NT_mid, 
                                                random = ~ 1 | study_id)
ml_meta_model_mod_child_age_NT_mid.M3
#RESULTS
#-- Child age during NT is still significant after entering time between NT and MA measure as covariates 
...
#RESULTS 
#Simple regression
#NT delivered at home has a larger effect (b = 0.3137, p = 0.0084)
...
```{r}
plot(influence(ml_meta_model), cex = 0.8, las = 1)
