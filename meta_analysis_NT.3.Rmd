...
######################################################################
#Leave 1 out analysis for rma.mv objects (require metafor)
...
######################################################################
#Leave 1 out analysis for rma.mv objects for categorical/dummy variables  (require metafor)
...
rma.mv.outlier <- function(data, struct = "CS", method = "REML", random, mods = NULL, b = 0, cluster){
  ...
  return(select(full_join(full_join(mutate(add_column(as.data.frame(cbind(leave1out, se, tval, pval, ci.lb, ci.ub)), rename(as.tibble(as.factor(c)), effect = value), .before = T),
                sig = ifelse(pval <= 0.05, ifelse(pval <= 0.01, ifelse(pval <= 0.001, "***", "**"), "*"), "ns")), cd.2, by = "effect"), dfbetas2, by = "effect"), effect, cooks.d, DFBETAS, leave1out, se, tval, pval, ci.lb, ci.ub, sig))
}
