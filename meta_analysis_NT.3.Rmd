New version (Not checked for bugs, please see the simplfied version, that definitely does not have bugs)
---
...
mix_et_al.2012_tib <- c(
  ...
  data.frame() %>% rename(., effect = .) %>% add_column(
    ...
    ) %>% mutate(r = d.rm/(sqrt((d.rm^2)+4))) %>%  select(effect, effect_description, effect_id, year, country, study_id, design, design.2, SES, main_language, deliverer_type, mothers, white, female, child_age_study_start, child_age_NT_start, child_age_NT_mid, child_age_NT_end, child_age_MA, NT_measure_time, time_between_NT_mid_MA, intervention_dosage, NT_context, MA_context, NT_size, NT_size.1, NT_type, NT_type.1, NT_type.10, MA_test, MA_measure, analysis, controlled_variables, r, n)

mix_et_al.2012_tib %>% view()


mix_et_al.2012_tib %>% pull("effect", "effect_id", 'time_between_NT_mid_MA')
cbind(mix_et_al.2012_tib$d.ppc, mix_et_al.2012_tib$time_between_NT_mid_MA, mix_et_al.2012_tib$d.rm) %>% as.tibble() %>% rename(., d.ppc = V1, time_between_NT_mid_MA = V2, d.rm = V3) %>% mutate(d.ppc = as.numeric(d.ppc), time_between_NT_mid_MA = as.factor(time_between_NT_mid_MA)) %>% aggregate(d.ppc ~ time_between_NT_mid_MA, FUN = mean)
r = d.ppc/sqrt((d.ppc^2) + 4)
#write.xlsx(mix_et_al.2012_tib, "mix_et_al.2012_tib.xlsx", overwrite = )

mixtib.p1 <- cbind(tm1.mix, ts1.mix, tm2.mix, ts2.mix, cm1.mix, cs1.mix, cm2.mix, cs2.mix, r.mix, n.mix) %>% as.tibble() %>% rename(tm1 = tm1.mix, ts1 = ts1.mix, tm2 = tm2.mix, ts2 = ts2.mix, cm1 = cm1.mix, cs1 = cs1.mix, cm2 = cm2.mix, cs2 = cs2.mix, r = r.mix, n = n.mix)
...
correlations <- c(0.50, 0.50, 0.67, 0.67, 0.95, 0.11, -0.26, 0.57, -0.13, -0.20, 0.61, 0.61, 0.70, 0.70, 0.81, -0.32, 0.67, 0.94, 0.46, 0.52) %>% as.tibble() %>% rename(r = value) %>% mutate(abs.r = abs(r))
correlations$r %>% median()
...
v_CHE <- impute_covariance_matrix(meta_tib$vi, cluster = meta_tib$study_id, r = c(0.59))
