...
ramani_et_al.2015_tib <- c(
  ...
  "ramani_et_al.2015l") %>% data.frame() %>% rename(., effect = .) %>% add_column(
    ...
    NT_type = c("Counting_and_Number Identificaton", "Cardinality_and_Ordering", "Counting_and_Number_Identification", "Cardinality_and_Ordering", "Counting_and_Number_Identification", "Cardinality_and_Ordering", "Counting_and_Number_Identification", "Cardinality_and_Ordering", "Counting_and_Number_Identification", "Cardinality_and_Ordering", "Counting_and_Number_Identification", "Cardinality_and_Ordering"),
...
boonen_et_al.2011_tib <- c(
  ...
  data.frame() %>% rename(., effect = .) %>% 
    add_column(
      ...
      NT_type = c("Calculation", "Convetional_Normatives", "Counting", "Ordering", "Number_Symbols", "Cardinality"),
...
meta_tib <- escalc(measure = "ZCOR", ri = r, ni = n, data = meta_tib.p1) %>% data.frame() %>% 
    ...
    mothers = as.factor(mothers),
    white = as.factor(white),
    female = as.factor(female),
    child_age_exp_start = as.numeric(child_age_exp_start),
...
rma.mv(yi = yi, V = vi, data = meta_tib, struct = "CS", method = "REML", test = "t", 
       random = list(~ 1 | sample_id/effect))
...
#NT_measure_time
#main_language
forest(ml_meta_model , addpred = F, header = T)

contrasts()

meta_tib %>% dplyr::mutate(MA_measure = forcats::fct_relevel(MA_measure, "Overall", "Calculation", "Cardinality", "Counting", "Magnitude_Comparison", "Mathematical_Problem_Solving", "Number_Identification", "Number_Knowledge", "Number_Sense"))
